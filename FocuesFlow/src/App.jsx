...
import { AccentColorProvider, useAccentColor } from './components/Context/AccentColorContext';
...
// Create theme based on dark mode and accent color
const createAppTheme = (isDarkMode, accentColor) => {
  ...
  return createTheme({
    ...
    components: {
        ...
        styleOverrides: {
          ...
          contained: {
            boxShadow: isDarkMode 
              ? `0 4px 24px rgba(${accentRgb}, 0.15)` 
      ...
      MuiMenu: {
        styleOverrides: {
          paper: {
            ...
            border: isDarkMode ? '1px solid #383B40' : '1px solid #E6E2DF',
          },
        },
      },
      MuiMenuItem: {
      ...
      MuiCheckbox: {
        styleOverrides: {
          root: {
            color: isDarkMode ? '#383B40' : '#D9CFC1',
            '&.Mui-checked': {
              color: accentColor,
            },
          },
      ...
      MuiSwitch: {
        styleOverrides: {
          ...
          track: {
            backgroundColor: isDarkMode ? '#383B40' : '#D9CFC1',
          },
        },
      },
      MuiTab: {
...
// AppContent component to use dark mode and accent color contexts
const AppContent = () => {
  const { isDarkMode } = useDarkMode();
  const { accentColor } = useAccentColor();
  
  // Memoize the theme to prevent unnecessary re-renders
  const theme = React.useMemo(() => createAppTheme(isDarkMode, accentColor), [isDarkMode, accentColor]);

  console.log('🔄 AppContent rendering with dark mode:', isDarkMode, 'accent color:', accentColor);
